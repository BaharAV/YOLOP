...
def detect(cfg,opt):
    ...
    with torch.no_grad():
      ...
      #print(f"Expected input image size: {img_size}")
      # Set Dataloader
      if opt.source.isnumeric():
          cudnn.benchmark = True  # set True to speed up constant image size inference
          dataset = LoadStreams(opt.source, img_size=img_size)
          bs = len(dataset)  # batch_size
      else:
          dataset = LoadImages(opt.source, img_size=img_size)
          bs = 1  # batch_size
      ...
      for i, (path, img, img_det, vid_cap,shapes) in tqdm(enumerate(dataset),total = len(dataset)):
          ...
          det_out, da_seg_out, ll_seg_out= model(img) #consider in speed calculation
          ...
          # Apply NMS #consider in speed calculation
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='1,2,3,4', help='cuda device, i.e. 0 or 0,1,2,3 or cpu') #update - cpu, or id of gpus
